...
def add_faculty(faculty):
    userAccount=models.User(email=faculty["email"],first_name=faculty["first_name"],last_name = faculty["last_name"],password = faculty["password"],username = faculty["username"])
...
def populate_time_table(slots):
    for slot in slots:
        try:
            ...
            timetable=models.Timetable.objects.get(lecture=lecture,day=slot["day"],start=slot["start"],duration=slot["duration"])

        except ObjectDoesNotExist as oe:
            timetable=models.Timetable(lecture=lecture,day=slot["day"],start=slot["start"],duration=slot["duration"])
            timetable.save()
...
def add_student(students):
    for student in students:
        try:
            ...
            stud_div=models.Division.objects.get(clas__dept__name=student["dept"],div=student["div"], clas__semester=student["sem"])
            try:
                user=models.User.objects.get(email=student["email"], first_name=student["first_name"],last_name=student["last_name"], password=student["password"],username=student["username"])
            except ObjectDoesNotExist as ode:
                user=models.User(email=student["email"], first_name=student["first_name"],last_name=student["last_name"], password=student["password"],username=student["username"])
                user.save()
